<x-app-layout>
    ...
    <div class="w-full h-full p-4 m-8 overflow-y-auto">
        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight" style=" margin-bottom: 5px; ">
            Client:  {{ $client->fname }} {{ $client->lname }} ID: ({{ $client->id }}) |  <b class="text-blue-500">LifeTime Points:  {{$lifetime_points->points}}</b>
                                 ...
                                 font-bold text-gray-600">New Points: </label>
                                    ...
                                    @if($client->new_points>=500)
                                        ...
                                        <div class="bg-white-100 border-t-4 border-white-500 rounded-b text-white-900 px-4 py-3 shadow-md" role="alert">
                                            <div class="flex">
                                                <form  id="redeem_500" class="w-full float-right" action="{{ url('redeem-client/'.$client->id).'/'.Auth::user()->id }}" method="Post" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type=number name="redeemed_points" value="500" style="display: none">
                                                    <input type="submit" id="redeem_btn_500" class=" text-xl	  p-2 cursor-pointer  text-blue-500 border border-blue-500 hover:bg-blue-500 hover:text-white active:bg-blue-600 font-bold  rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" value="Redeem voucher -500 points | 25% on all items"/>
                                                </form>
                                            </div>
                                        </div>
                                    @endif

                                    @if($client->new_points>=300)

                                        <div class="bg-white-100 border-t-4 border-white-500 rounded-b text-white-900 px-4 py-3 shadow-md" role="alert">
                                            <div class="flex">
                                                <form  id="redeem_300" class="w-full float-right" action="{{ url('redeem-client/'.$client->id).'/'.Auth::user()->id }}" method="Post" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type=number name="redeemed_points" value="300" style="display: none">
                                                    <input type="submit" id="redeem_btn_300" class=" text-xl	  p-2 cursor-pointer  text-green-500 border border-green-500 hover:bg-green-500 hover:text-white active:bg-green-600 font-bold  rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" value="Redeem voucher -300 points | 15% on all items"/>
                                                </form>
                                            </div>
                                        </div>
                                    @endif

                                    @if($client->new_points>=50)

                                        <div class="bg-white-100 border-t-4 border-white-500 rounded-b text-white-900 px-4 py-3 shadow-md" role="alert">
                                            <div class="flex">
                                                <form  id="redeem_50" class="w-full float-right" action="{{ url('redeem-client/'.$client->id).'/'.Auth::user()->id }}" method="Post" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type=number name="redeemed_points" value="50" style="display: none">
                                                    <input type="submit" id="redeem_btn_50" class=" text-xl	  p-2 cursor-pointer  text-fuchsia-400 border border-fuchsia-400 hover:bg-fuchsia-400 hover:text-white active:bg-fuchsia-400 font-bold  rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" value="Redeem voucher -50 points |  free coffee"/>
                                                </form>
                                            </div>
                                        </div>
                                    @endif
